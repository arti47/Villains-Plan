...
test("every library entry with a citation cites this source", () => {
  const lib = rules.libraryGroups().flatMap((g) => g.entries);
  for (const entry of lib) if (entry.cite) assert(/^MM69:p/.test(entry.cite), `${entry.id} cites ${entry.cite}`);
  assert(lib.length >= 14, "the library covers every automated rule");
});
