...
| A40 | When the conclusion's guarantee is spent | "Don't test the Expected Scene against the Chaos Factor" applies to the one scene the delayed conclusion is played in. The app marks the track `conclusionPlayed` as that scene opens, so the scene after it is tested normally. |
| A37 | The Discovery Fate Question | Not a plain yes/no gate. An **Exceptional Yes rolls the table twice and combines**; an **Exceptional No** finds nothing *and* bars further Discovery Checks for the rest of the scene. The app stores that on the scene, so the next scene opens Discovery again — the book's own "must continue such searches in another Scene". |
...
| `data-fate-chart.js` | GME2e: the Fate Chart's 81 cells, the three variant charts' 81, and the two equivalences the harness checks them against |
...
| MM41 names seven detail tables | — | `VILLAIN_DETAIL_TABLES` | `rules.villainDetailTables` | Those seven as multi-select chips with an "all seven" control, the rest behind a fold | `the seven tables The Villain Crafter names are all present`, `the details step rolls every table picked, each on its own d100` |
...
| Each chaos variant has its own chart and its own Check ladder | Lookup | `CHART_VARIANTS`, `FATE_CHECK.midChaosModifiers` / `lowChaosModifiers` | `rules.chartFor` → `rules.fateBands`; `rules.checkChaosModifier` | Chaos-mode chips; the odds card names the chart and column; the four charts compared on the Rules tab | `the three variant Fate Charts are transcribed, and each is a slice of the standard chart`, `a chaos mode changes which chart is read`, `the Low-Chaos Fate Check modifiers match the page` |
| An Exceptional No bars Discovery for the rest of the scene | Gate | `PROGRESS_TRACK.discovery.answers` | `scenes.discoveryBlock` → `store.updateScene` | Refusal on the track card, cleared by the next scene | `an Exceptional No shuts Discovery down for the rest of the scene, and only that scene` |
| An Exceptional Yes rolls the Discovery table twice | Exception | `PROGRESS_TRACK.discovery.answers` | `scenes.rollDiscovery` ×2 | Both results named on the track card | `the Discovery Fate Question decides how many times the table is rolled` |
| Discovery Check: ask ≥50/50, then 1d10 + points; all eight results award | Lookup | `PROGRESS_TRACK.discovery` | `scenes.discoveryBlock` → `scenes.rollDiscovery` | Track card, with meaning words because the roll *is* a random event | `covers its range and only awards what the source states` |
| A phase of five points without a flashpoint gets one | Escalation + Exception | `PROGRESS_TRACK.phaseSize`, `phaseFlashpoint` | `derived.trackPhases` / `owedFlashpoint` → `scenes.scoreTrack` | Phase chips + "did a flashpoint happen?" | `the track is phases of five, and a phase without a flashpoint is owed one`, `a flashpoint inside a phase satisfies it` |
| Plot armour covers a Close A Thread event | Gate | `PROGRESS_TRACK.plotArmor` | `oracle.eventBlock` → `armouredFocusThread` | Warning on the event block | — (surfaced; the gate itself is tested under plot armour) |
| The delayed conclusion's scene is not tested | Exception | `PROGRESS_TRACK.conclusionDelay` | `scenes.carriedConclusion` → `testScene` | Scene card says it begins as imagined | `a delayed conclusion is not tested against the Chaos Factor` |
...
| 2026-09-11 | The Thread Progress Track chapter: Track and Strengthen Progress defined (all eight Discovery results award), phases of five with the compulsory flashpoint and its two timings, plot armour against a Close A Thread event, and the conclusion's untested scene. **`NOT_SUPPLIED` is now empty, and the suite asserts it** | The user supplied the chapter; it closed the last gap | `npm test` 163, `npm run smoke` 547, `npm run interaction` 461 | `schemer-v13` |
| 2026-09-11 | The Mid-Chaos, Low-Chaos and No-Chaos Fate Charts (81 cells), the Low-Chaos Check modifiers, the Discovery Fate Question's four branches, and the quoted list-selection procedure. Mid-Chaos is no longer refused on the chart; Low-Chaos is new; the last `provisional` flag is gone. A31 corroborated, A35's hypothesis confirmed, A36 and A37 added. F46: a 5%-flaky test | The user supplied the pages | `npm test` 160 (30 consecutive clean runs), `npm run smoke` 547, `npm run interaction` 463 | `schemer-v12` |
